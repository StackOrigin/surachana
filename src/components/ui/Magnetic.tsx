...
export default function Magnetic({ children, className }: { children: ReactNode; className?: string }) {
  ...
  const move = (event: React.PointerEvent<HTMLDivElement>) => {
    ...
    ref.current?.style.setProperty('--magnetic-transform', `translate3d(${x}px, ${y}px, 0)`);
  };

  const reset = () => ref.current?.style.setProperty('--magnetic-transform', 'translate3d(0, 0, 0)');
